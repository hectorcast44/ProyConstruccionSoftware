...
const UIHelpers = (() => {
  ...
  const TagStyleManager = {
    ...
    /**
     * Obtiene la clase CSS para un tipo de actividad basado en el nombre.
     * Mapea palabras clave a clases de color predefinidas.
     *
     * @param {string|number} key Identificador o nombre del tipo de actividad.
   ...
   * Soporta tanto la estructura antigua (.materias-grid) como la nueva (.accordion-card-grid).
   *
   * @param {HTMLElement} [container] Contenedor del grid (opcional). Si no se proporciona, busca .accordion-card-grid o .materias-grid
   * @returns {void}
   */
  function initAccordionGrid(container) {
    // Si no se pasa contenedor, buscar por selectores conocidos
    let grid = container;
    if (!grid) {
    ...
    grid.addEventListener('click', (e) => {
      // Manejo de menú contextual (tres puntos) - estructura nueva
      ...
      // Manejo de menú contextual (tres puntos) - estructura antigua
      ...
      // Manejo del acordeón (expandir/contraer tarjeta) - estructura nueva
      ...
      // Manejo del acordeón (expandir/contraer tarjeta) - estructura antigua
    ...
    document.addEventListener('click', (e) => {
      if (!e.target.closest('.accordion-card__menu') && !e.target.closest('.accordion-card__menu-toggle') && !e.target.closest('.materia-menu')) {
        document.querySelectorAll('.accordion-card__menu').forEach(m => {
          m.style.display = 'none';
        });
        document.querySelectorAll('.materia-menu-content.show').forEach(m => {
   ...
   * Soporta tanto estructura antigua como nueva.
   *
   * @param {Object} options Opciones de configuración
   * @param {HTMLElement} [options.input] Input de búsqueda
   * @param {HTMLElement} [options.toggleBtn] Botón para toggle del buscador
   * @param {HTMLElement} [options.wrapper] Wrapper del buscador
   * @param {Function} [options.onFilter] Callback para filtrado personalizado
   * @returns {void}
   */
  function initSearchBar(options = {}) {
    ...
    input.addEventListener('input', (e) => {
      ...
      if (typeof onFilter === 'function') {
        ...
        // Filtrado automático para estructura antigua
        ...
        cards.forEach(card => {
          ...
          const matches = title.includes(term.toLowerCase().trim()) || tags.some(tag => tag.includes(term.toLowerCase().trim()));
